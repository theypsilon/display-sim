...
pub trait HasContext {
    type Shader: Default + Copy + Clone;
    type Program: Default + Copy + Clone;
    type Buffer: Default + Copy + Clone;
    type VertexArray: Default + Copy + Clone;
    type Texture: Default + Copy + Clone;
    type Sampler: Default + Copy + Clone;
    type Fence: Default + Copy + Clone;
    type Framebuffer: Default + Copy + Clone;
    type Renderbuffer: Default + Copy + Clone;
    type UniformLocation: Default + Copy + Clone;
}
#[derive(Copy, Clone)]
pub struct Context {}
#[derive(Copy, Clone)]
...
impl<GL: HasContext> GlowSafeAdapter<GL> {
    ...
    pub fn clear_depth_f64(&self, _: f64) {}
    pub fn clear_depth_f32(&self, _: f32) {}
    pub fn clear_stencil(&self, _: i32) {}
