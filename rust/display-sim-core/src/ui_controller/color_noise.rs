...
impl UiController for ColorNoise {
    ...
    fn update(&mut self, speed: f32, ctx: &dyn SimulationContext) -> bool {
        FieldChanger::new(ctx, &mut self.value, self.input)
            ...
            .set_min(0.0)
            .set_max(1.0)
            .set_trigger_handler(|x| dispatch_color_noise(x, ctx.dispatcher()))
            .process_with_sums()
    }
    ...
    fn dispatch_event(&self, dispatcher: &dyn AppEventDispatcher) {
        dispatch_color_noise(self.value, dispatcher)
    }
...
fn dispatch_color_noise(value: f32, dispatcher: &dyn AppEventDispatcher) {
